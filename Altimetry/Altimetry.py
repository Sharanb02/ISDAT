...
class ATL06:
    ...
    def __init__(self,usrnm,email):

        self.usrnm = usrnm
        self.email = email
        self.glims = gpd.read_file('C:/Users/shara/ICESAT/glims_him/glims_him.shp')
        
    def get_data(self,bbox,date_range,path) :
        ...
        region.earthdata_login(self.usrnm,self.email)
    ...
    def h_li_plot(self,region,end_time,rel_del=3):
        ...
        if region in ['Karakoram','West Himalaya','East Himalaya','Central Himalaya']:
            ...
            beam_gdf1.plot('h_li',cmap='BuPu',markersize=0.1,ax=ax,legend=True,cax=cax_,label="land",vmin=3000,vmax=7000)
            beam_gdf2.plot('h_li',ax=ax,cmap='BuPu',markersize=0.1,vmin=3000,vmax=7000)
            beam_gdf3.plot('h_li',ax=ax,cmap='BuPu',markersize=0.1,vmin=3000,vmax=7000)
            ax.set_title("Land Ice Height (CRS:WGS 84 EPSG:4326)",font='Times New Roman')
            left, width = .15,.5
            bottom, height = .1, .5
            right = left + width
            top = bottom + height
            ax.text(left,bottom,region,horizontalalignment='left', verticalalignment='center',transform=ax.transAxes,bbox=dict(facecolor='gainsboro',alpha=0.3),font='Times New Roman')
...
class GLAH06:
    ...
    def get_data(self,bbox,date_range,path):
        ...
        response = session.get(s.url,auth=(self.usrnm,self.pswd))
    ...
    def ele_plot(self,region_path,end_time,rel_del=3):
        
        year, month, day = map(int, end_time.split('-'))
        start_time = date(year, month, day)+relativedelta(months=-rel_del)+relativedelta(days=+1)
        start_time = str(start_time)
        print(start_time)
        date_range=[start_time,end_time]
        ...
        glims_sia_1 = self.glims[self.glims['glac_id']=='G077129E35401N']
        glims_sia = glims_sia_1[glims_sia_1['line_type']=='glac_bound']
        glims_sia_rk = glims_sia_1[glims_sia_1['line_type']=='intrnl_rock']
        if (os.path.isdir(data_path+'/'+date_range[0]+'--'+date_range[1])==False) or (len(os.listdir(data_path+'/'+date_range[0]+'--'+date_range[1]))<=1):
            print('Downloading Data...')
            self.get_data(shp,date_range,data_path)
            self.data_to_csv(data_path+'/'+date_range[0]+'--'+date_range[1])
        else:
            print("Data aldready exists")
        ...
        glims_sia_rk.plot(column='line_type',categorical=True,ax=ax,color='gray',alpha=0.2,legend=True)
        divider = make_axes_locatable(ax)
        cax_ = divider.append_axes("right", size="5%", pad=0.05)
        cat_= divider.append_axes("bottom", size="5%", pad=0.5)
        ...
        right = left + width
        top = bottom + height
        ax.text(left,bottom,'Siachen Glacier',horizontalalignment='left', verticalalignment='center',transform=ax.transAxes,bbox=dict(facecolor='gainsboro',alpha=0.3),color='white',font='Times New Roman')                       
        beam1.plot('ele',cmap='Blues',markersize=0.1,ax=ax,legend=True,cax=cax_,vmin=3000,vmax=7000)
        fig.savefig('C:/Users/shara/ISDAT/SURGE/Siachen/Timeseries/'+date_range[0]+'--'+date_range[1]+'.png')
